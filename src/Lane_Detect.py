...
def func_Region_of_Interest_Mask(image):
    ...
    image_with_ROI = cv.bitwise_and(image, mask) #it does bit wise and operation all over the image to give region of interest
...
video  = cv.VideoWriter('output/Lane _Detection_output_video.avi', cv.VideoWriter_fourcc(*'XVID'), 10,(960,540))
...
while True: #enter while loop to read video frame by frame
    ...
    lines = cv.HoughLinesP(image_with_ROI, 1, np.pi/180, threshold = 60, minLineLength = 10, maxLineGap = 400)
    ...
    #now we find out the lowest and highest points on the line using our slope and intercept
    y_1 = 303 #this value has been calculated so as to show that line starts from the vanishing point of lane
    x_1 = int((y_1-intercept)/slope)
    y_2 = int(frame.shape[0])
    x_2 = int((y_2-intercept)/slope)    

    cv.line(frame, (x_1,y_1),(x_2,y_2), (0,0,255),10) #red for dashed line
    f = frame
    video.write(f)  #save the video frame by frame
    cv.imshow('Lanes', f) #display the video frame by frame
